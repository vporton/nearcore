...
impl<'a> ChainStoreAccess for ChainStoreUpdate<'a> {
    ...
    fn get_outcomes_by_id(
        ...
        self.chain_store.get_outcomes_by_id(id)
    }
...
impl<'a> ChainStoreUpdate<'a> {
    ...
    pub fn gc_outcomes(&mut self, block_hash: &CryptoHash) -> Result<(), Error> {
        let mut store_update = self.store().store_update();
        ...
        self.gc_col(ColOutcomesByBlockHash, &block_hash.as_ref().into());
        self.merge(store_update);
        Ok(())
    }
